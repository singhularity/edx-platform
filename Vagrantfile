...
if [ -n "$OPENEDX_RELEASE" ]; then
  EXTRA_VARS="-e certs_version=$OPENEDX_RELEASE \
    -e forum_version=$OPENEDX_RELEASE \
    -e xqueue_version=$OPENEDX_RELEASE \
  "
fi

if [ -n "$AMPLIFY_BRANCH" ]; then
  BRANCH_VER=$AMPLIFY_BRANCH
  ...
  /edx/bin/update configuration $BRANCH_VER
else
  BRANCH_VER="amplify-master"
fi

ansible-playbook -i localhost, -c local vagrant-devstack.yml --tags=deploy -e configuration_version=$BRANCH_VER -e edxapp_version=$BRANCH_VER $EXTRA_VARS

SCRIPT

edx_platform_mount_dir = "edx-platform"
themes_mount_dir = "edxamplifytheme"
...
if ENV['VAGRANT_MOUNT_BASE']
  ...
  config_mount_dir = ENV['VAGRANT_MOUNT_BASE'] + "/"  + config_mount_dir
...
branch = "${ENV['EDX_BRANCH']:-amplify-master}"
...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  config.vm.provision "shell", inline: $script, args: "$rel $branch"
end
