...
class DjangoSudoStartupTestCase(TestCase):
    ...
    def test_run_third_party_auth(self, is_enable):
        with patch.dict("django.conf.settings.FEATURES", {"ENABLE_THIRD_PARTY_AUTH": is_enable}):
            self.assertEqual(settings.FEATURES["ENABLE_THIRD_PARTY_AUTH"], is_enable)
